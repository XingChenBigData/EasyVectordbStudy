...
部署参考：https://github.com/datawhalechina/all-in-rag/blob/main/docs/chapter3/09_milvus.md。第四节 Milvus介绍及多模态检索实践
官网部署参考：https://milvus.io/docs/zh/install_standalone-docker-compose.md
```bash
wget https://github.com/milvus-io/milvus/releases/download/v2.6.8/milvus-standalone-docker-compose.yml -O docker-compose.yml

sudo docker compose up -d
```
官网数据库操作参考：https://milvus.io/docs/zh/manage_databases.md

## 一、简介

Milvus 是一个开源的、专为大规模向量相似性搜索和分析而设计的向量数据库。 
Milvus 从设计之初就瞄准了**生产环境**。
...
**第一步：下载配置文件**
`docker-compose.yml` 文件。定义了 Milvus Standalone 及其运行所需的两个核心依赖服务：`etcd` 用于存储元数据，`MinIO` 用于对象存储（更多架构细节请参考[官方文档](https://milvus.io/docs/architecture_overview.md)）。
